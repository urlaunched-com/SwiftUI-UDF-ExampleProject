...
public final class RecommendationsMiddleware<F: RecommendationsFeature>: Middleware<F>, @unchecked Sendable {
    ...
    public func scope(for state: F) -> Scope {
        ...
        state.recommendationsBindableFlow
    }
    ...
    public func observe(state: F) {
        for (id, flow) in state.recommendationsBindableFlow {
            switch (id, flow) {
            case let (.movie(movieID), .load(page)):
                execute(
                    ...
                    flowId: RecommendationsFlow.id,
                    cancellation: Cancellation.load(id),
                    mapAction: {
                        $0.binded(to: F.RecomendationsContainerType.self, by: id)
                    }
                )
...
public extension RecommendationsMiddleware {
...
private extension RecommendationsMiddleware {
    struct LoadMoviesEffect: ConcurrencyEffect {
        let movieId: Movie.ID?
        let page: Int
        let environment: RecommendationsMiddleware.Environment
